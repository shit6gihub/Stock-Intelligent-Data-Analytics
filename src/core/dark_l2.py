# -*- coding: utf-8 -*-
"""
L2 逐笔数据源(thsdk 实现, 2026-08-19)
========================================

把 dark_flow.py 预留的 `fetch_l2_ticks(code, source)` 占位替换为真实实现:
从同花顺 thsdk 拉取 tick_super_level1(约 3 秒累计条),差分还原后返回与腾讯逐笔
**同构**的列表:

    [{"d": "B"/"S"/"M", "amt": 金额(元), "vol": 手数, "price": 价格, "t": "HH:MM:SS"}]

接入方式: 设置环境变量 `PANWATCH_DARK_SOURCE=thsdk` 后 dark_flow 自动分发到本模块。
本模块不 import thsdk(惰性加载): 仓库主进程(未装 thsdk)导入本模块不会报错,
dark_flow 捕获异常后自动回退腾讯逐笔。

**实测口径(2026-08-19 国内服务器 101.35.244.238, thsdk 1.7.18 游客模式)**:
...
def fetch_l2_ticks(code: str, source: str = "thsdk") -> list[dict]:
    ...
    Args:
        ...
        source: 数据源标识, 仅支持 "thsdk"; 其他值走旧的 NotImplementedError 提示
    ...
    if source != "thsdk":
        raise NotImplementedError(
            ...
            "其他付费 L2 需先购买相应数据服务(见 dark_flow.DARK_SOURCE 注释)。"
        )
    ...
    for r in valid_rows:
        ...
        # 该时段的真实成交(如 9:25:00 神剑 5357500 股)统一标 M 中性, 不进主动 Delta。
        ts = r["时间"]
        hm = _ts_to_hms(ts)
        if "09:15" <= hm < "09:30":
            d = "M"
