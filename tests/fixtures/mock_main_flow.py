...
def fake_auction_df() -> pd.DataFrame:
    """竞价异动 DataFrame(列名模拟同花顺 2026-08-24 实测返回)。

    实测 thsdk get_auction_anomaly 只返回 ~6 列:
    时间 / 价格 / 总金额 / 代码 / 名称 / 异动类型1
    - 价格 / 总金额 是占位脏数据, 在 auction_pool 内仅用于 (价格/昨收 - 1)*100 推导 gap_pct。
    - 撤单率 / 量比 列已不存在(数据源不提供), 对应 record 字段固定 None。
    """
    return pd.DataFrame(
        [
            {
                "时间": "09:25:00",
                "价格": 8.32,            # 占位脏数据:真实 gap_pct 由 (价格/昨收 - 1)*100 算
                "总金额": 5357.5,
                "代码": "002361",
                "名称": "神剑股份",
                "异动类型1": "高开",
...
def fake_auction_df_with_legacy_cols() -> pd.DataFrame:
    """兼容旧列名(高开幅度/撤单率/量比)的 fake DataFrame。

    仅作"列名兼容"测试用: 验证 _to_records 仍能从老 DataFrame 抽出字段。
    """
    return pd.DataFrame(
        [
