...
def _db_with(logs, run=None, owner=None) -> MagicMock:
    ...
    """
    db = MagicMock()
    # 1. 归属校验 query(AgentRun).filter(...).first()
...
def test_not_owner_trace_not_found():
    ...
    # 归属校验 first()=None → 直接 return not_found, 不触发后续 query
