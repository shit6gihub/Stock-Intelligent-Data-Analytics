...
### feat(nav) — 持仓换成报告 + 报告页动态刷新

- 导航一级移除"持仓"(/portfolio), "报告"上移第2位; 桌面组/移动端底部/快捷键 g→p 同步
- /portfolio 路由保留(Dashboard 组合速览条"持仓页 →"入口仍在), /stocks 旧链接重定向不变
- Reports 页 30s 自动刷新(visibility 暂停 + 防叠加 + 静默更新不闪屏), 顶部"最后更新 HH:MM:SS"

### 实测
- build 8.49s, 导航 12 项顺序正确, 报告页自动刷新机制验证
